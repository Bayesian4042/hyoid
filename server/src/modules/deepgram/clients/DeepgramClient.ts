...
import { DeepgramConfig, SynthesisOptions, TranscriptionOptions } from '../types';
...
export class DeepgramClient {
  ...
  createLiveTranscription(options: Partial<TranscriptionOptions> = {}) {
    const transcriptionOptions: any = {
      ...
      sample_rate: options.sampleRate || this.defaultTranscriptionOptions.sampleRate,
      ...
      smart_format: options.smartFormat ?? this.defaultTranscriptionOptions.smartFormat,
      language: options.language || this.defaultTranscriptionOptions.language,
      detect_language: options.detectLanguage || this.defaultTranscriptionOptions.detectLanguage,
      punctuate: options.punctuate ?? this.defaultTranscriptionOptions.punctuate,
      profanity_filter: options.profanityFilter || this.defaultTranscriptionOptions.profanityFilter,
    };
   ...
   */
  async synthesizeAudio(text: string, options: Partial<SynthesisOptions> = {}): Promise<Buffer> {
    const synthOptions = {
   ...
   */
  createRealtimeTranscription(options: Partial<TranscriptionOptions> = {}, 
    handlers: {
